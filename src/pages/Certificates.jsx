...
const certificates=[ 
{id: 1,studentName: 'Emma Johnson',courseName: 'Nursery 1 Completion',issueDate: '2024-01-15',certificateId: 'LHL-2024-001',template: 'basic',grade: 'Excellent'},
{id: 2,studentName: 'Liam Chen',courseName: 'Toddler Program',issueDate: '2024-01-20',certificateId: 'LHL-2024-002',template: 'basic',grade: 'Very Good'},
{id: 3,studentName: 'Olivia Rodriguez',courseName: 'Kindergarten',issueDate: '2024-01-25',certificateId: 'LHL-2024-003',template: 'advanced',grade: 'Outstanding'},
{id: 4,studentName: 'Noah Williams',courseName: 'Nursery 2',issueDate: '2024-02-01',certificateId: 'LHL-2024-004',template: 'basic',grade: 'Good'},
{id: 5,studentName: 'Sophia Davis',courseName: 'Childcare Program',issueDate: '2024-02-10',certificateId: 'LHL-2024-005',template: 'basic',grade: 'Very Good'},
{id: 6,studentName: 'Mason Brown',courseName: 'Tutorial Classes',issueDate: '2024-02-15',certificateId: 'LHL-2024-006',template: 'tutorial',grade: 'Excellent'} 
];
...
const matchesSearch=cert.studentName.toLowerCase().includes(searchTerm.toLowerCase()) || cert.courseName.toLowerCase().includes(searchTerm.toLowerCase());
...
<div className="bg-white border-4 border-yellow-400 rounded-lg p-8 text-center shadow-lg"> 
<div className="mb-6"> 
...
<input type="text" placeholder="Search by student name or course..." value={searchTerm} onChange={(e)=> setSearchTerm(e.target.value)} className={`w-full pl-10 pr-4 py-3 rounded-lg border transition-colors duration-200 ${darkMode ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'} focus:outline-none focus:ring-2 focus:ring-primary-500`} /> 
</div> 
...
<select value={selectedYear} onChange={(e)=> setSelectedYear(e.target.value)} className={`px-4 py-3 rounded-lg border transition-colors duration-200 ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'} focus:outline-none focus:ring-2 focus:ring-primary-500`} > 
<option value="all">All Years</option> 
{years.slice(1).map((year)=> ( 
<option key={year} value={year}>{year}</option> 
))} 
</select> 
<select value={selectedCourse} onChange={(e)=> setSelectedCourse(e.target.value)} className={`px-4 py-3 rounded-lg border transition-colors duration-200 ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'} focus:outline-none focus:ring-2 focus:ring-primary-500`} > 
...
<motion.div key={certificate.id} initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} transition={{delay: index * 0.1}} className={`p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 ${darkMode ? 'bg-gray-800' : 'bg-white'}`} > 
...
<button className="flex-1 inline-flex items-center justify-center px-4 py-2 bg-soft-rose text-white rounded-lg hover:shadow-md transition-all duration-200 text-sm font-medium"> 
<SafeIcon icon={FiEye} className="mr-2 w-4 h-4" /> 
View 
</button> 
<button className={`flex-1 inline-flex items-center justify-center px-4 py-2 rounded-lg hover:shadow-md transition-all duration-200 text-sm font-medium ${darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}> 
...
<motion.div key={index} initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} transition={{delay: index * 0.1}} className={`text-center p-6 rounded-2xl shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`} > 
<div className="w-16 h-16 bg-soft-rose rounded-full flex items-center justify-center mx-auto mb-4"> 
