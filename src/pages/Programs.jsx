...
const matchesAge=selectedAge==='all' || course.ageGroup.includes(selectedAge) || (selectedAge==='all-ages' && course.ageGroup==='All ages');
const matchesType=selectedType==='all' || (selectedType==='full-time' && course.duration.includes('Full')) || (selectedType==='part-time' && course.duration.includes('Half')) || (selectedType==='tutorial' && course.title.includes('Tutorial'));
...
<select value={selectedAge} onChange={(e)=> setSelectedAge(e.target.value)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${darkMode ? 'bg-gray-700 text-gray-300 border border-gray-600' : 'bg-white text-gray-700 border border-gray-300'} focus:outline-none focus:ring-2 focus:ring-primary-500`} > 
{ageGroups.map((age)=> ( 
<option key={age.value} value={age.value}>{age.label}</option> 
))} 
</select> 
<select value={selectedType} onChange={(e)=> setSelectedType(e.target.value)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${darkMode ? 'bg-gray-700 text-gray-300 border border-gray-600' : 'bg-white text-gray-700 border border-gray-300'} focus:outline-none focus:ring-2 focus:ring-primary-500`} > 
...
<motion.div key={course.id} initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} transition={{delay: index * 0.1}} className={`rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 ${darkMode ? 'bg-gray-800' : 'bg-white'}`} > 
<div className="h-56 overflow-hidden"> 
<img src={course.image} alt={course.title} className="w-full h-full object-cover" /> 
</div> 
<div className="p-6"> 
<div className="flex items-center justify-between mb-4"> 
<span className="px-3 py-1 bg-soft-rose text-white rounded-full text-sm font-medium"> 
{course.ageGroup} 
</span> 
...
<Link to="/enrollment" className="px-4 py-2 bg-soft-rose text-white rounded-lg hover:shadow-md transition-all duration-200 text-sm font-medium"> 
Enroll Now 
</Link> 
</div> 
</div> 
...
<motion.div key={index} initial={{opacity: 0,x: -20}} whileInView={{opacity: 1,x: 0}} transition={{delay: index * 0.1}} className="flex items-center space-x-3" > 
...
<img src="https://images.unsplash.com/photo-1503454537195-1dcabb73ffb9?w=600&h=500&fit=crop" alt="Children learning together" className="rounded-2xl shadow-xl w-full h-96 object-cover" /> 
...
<motion.p className={`text-xl ${darkMode ? 'text-gray-400' : 'text-gray-600'} max-w-3xl mx-auto`} initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} transition={{delay: 0.2}} > 
Numbers that speak to our commitment to excellence 
</motion.p> 
</div> 
<div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8"> 
{[ 
{number: '6',label: 'Program Offerings',icon: FiBookOpen},
...
{/* Call to Action */} 
<section className={`py-20 ${darkMode ? 'bg-gray-700' : 'solid-bg-dusty-blue'}`}> 
<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center"> 
<motion.h2 className="font-display font-bold text-3xl lg:text-4xl text-white mb-6" initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} > 
Ready to Enroll Your Child? 
</motion.h2> 
<motion.p className="text-xl text-white/90 mb-8 max-w-3xl mx-auto" initial={{opacity: 0,y: 20}} whileInView={{opacity: 1,y: 0}} transition={{delay: 0.2}} > 
...
<Link to="/enrollment" className="px-8 py-4 bg-white text-dusty-blue font-semibold rounded-full hover:bg-gray-100 transition-all duration-300 hover:shadow-lg hover:scale-105" > 
📩 Enroll Now 
</Link> 
<Link to="/contact" className="px-8 py-4 border-2 border-white text-white font-semibold rounded-full hover:bg-white hover:text-dusty-blue transition-all duration-300 hover:shadow-lg hover:scale-105" > 
📅 Schedule a Visit 
</Link> 
</motion.div> 
</div> 
